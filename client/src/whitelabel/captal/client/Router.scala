...
object Router:
  // Route patterns
  private val welcomeRoute: Route[WelcomePage.type, Unit] =
    Route.static(WelcomePage, root / endOfSegments)

  private val questionRoute: Route[IdentificationQuestionPage.type, Unit] =
    Route.static(IdentificationQuestionPage, root / "question" / endOfSegments)

  private val advertiserVideoRoute: Route[AdvertiserVideoPage.type, Unit] =
    Route.static(AdvertiserVideoPage, root / "final" / endOfSegments)

  private val readyRoute: Route[ReadyPage.type, Unit] =
    Route.static(ReadyPage, root / "ready" / endOfSegments)

  private object router extends com.raquo.waypoint.Router[Page](
    routes = List(welcomeRoute, questionRoute, advertiserVideoRoute, readyRoute),
    getPageTitle = {
      case WelcomePage                => "WiFi Gratis"
      case IdentificationQuestionPage => "Pregunta"
      case AdvertiserVideoPage        => "Final"
      case ReadyPage                  => "Listo"
    },
    serializePage = {
      case WelcomePage                => "welcome"
      case IdentificationQuestionPage => "question"
      case AdvertiserVideoPage        => "final"
      case ReadyPage                  => "ready"
    },
    deserializePage = {
      case "welcome"  => WelcomePage
      case "question" => IdentificationQuestionPage
      case "final"    => AdvertiserVideoPage
  ...
  private def phaseToPage(phase: Phase): Page =
    phase match
      case Phase.Welcome                => WelcomePage
      case Phase.IdentificationQuestion => IdentificationQuestionPage
      case Phase.AdvertiserVideo        => AdvertiserVideoPage
      case Phase.AdvertiserQuestion     => AdvertiserVideoPage // For now, redirect to video
      case Phase.Ready                  => ReadyPage
