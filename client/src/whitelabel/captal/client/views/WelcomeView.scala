...
object WelcomeView:
    ...
    Layout(
      ...
      footer = div(
          ...
          children <-- availableLocales.signal.map: locales =>
            locales.map: loc =>
              option(
                value := loc,
                loc.toUpperCase,
                selected <-- I18nClient.locale.map(_ == loc)
              ),
        ...
        button(
          ...
          child.text <-- isStarting.signal.combineWith(I18nClient.i18n).map:
            case (true, i18n) => i18n.welcome.button.connecting
            case (false, i18n) => i18n.welcome.button.start,
          onClick --> { _ => startFlow() }
        )
      )
    )

  private def step(textSignal: Signal[String], index: Int): HtmlElement =
    div(
      cls := "step",
  ...
  private def loadLocales(): Unit =
    Runtime.run:
      for
        ...
        locale = statusResult match
          case Right(status) =>
            // If phase is not Welcome, redirect to the correct page
            if status.phase != Phase.Welcome then Router.syncWithPhase(status.phase)
            status.locale
          case Left(_) => detectBrowserLocale()
  ...
  private def detectBrowserLocale(): String =
    ...
    if browserLang.startsWith("es") then "es"
    else if browserLang.startsWith("en") then "en"
    else "es" // default
