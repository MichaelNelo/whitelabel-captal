...
object IdentificationQuestionView:
  ...
  private def surveyErrorToMessage(error: SurveyError): String =
    ...
    error match
      case SurveyError.RequiredAnswerMissing(_) =>
        q.required
      case SurveyError.InvalidEmail(_) =>
        q.invalidEmail
      case SurveyError.InvalidUrl(_) =>
        q.invalidUrl
      case SurveyError.InvalidPattern(_, _) =>
        q.invalidPattern
      case SurveyError.TextTooShort(_, min, _) =>
        q.minLength.replace("{min}", min.toString)
      case SurveyError.TextTooLong(_, max, _) =>
        q.maxLength.replace("{max}", max.toString)
      case SurveyError.TooFewSelections(_, min, _) =>
        q.minSelections.replace("{min}", min.toString)
      case SurveyError.TooManySelections(_, max, _) =>
        q.maxSelections.replace("{max}", max.toString)
      case SurveyError.InvalidOptionSelected(_, _) =>
        q.invalidOption
      case SurveyError.InvalidOptionsSelected(_, _) =>
        q.invalidOption
      case SurveyError.RatingOutOfRange(_, min, max, _) =>
        q.ratingOutOfRange.replace("{min}", min.toString).replace("{max}", max.toString)
      case SurveyError.NumericOutOfRange(_, min, max, _) =>
        q.numericOutOfRange.replace("{min}", min.toString).replace("{max}", max.toString)
      case SurveyError.DateOutOfRange(_, min, max, _) =>
        q.dateOutOfRange.replace("{min}", min.toString).replace("{max}", max.toString)
      case SurveyError.IncompatibleAnswerType(_, _) =>
        q.invalidAnswer
      case _ =>
        I18nClient.current.error.generic
    end match
  end surveyErrorToMessage
  ...
  private def renderRadio(
      ...
      survey: NextIdentificationSurvey): HtmlElement = div(
    cls := "radio-group",
    options
  ...
  private def renderSelect(
      options: List[QuestionOption],
      survey: NextIdentificationSurvey): HtmlElement = select(
    cls := "select-input",
  ...
  private def renderCheckbox(
      options: List[QuestionOption],
      survey: NextIdentificationSurvey): HtmlElement =
  ...
  private def renderRating(_survey: NextIdentificationSurvey): HtmlElement =
    // TODO: Implement rating component (stars or slider)
    div(cls := "rating-input", "Rating input - TODO")

  private def renderNumeric(survey: NextIdentificationSurvey): HtmlElement = input(
    cls := "numeric-input",
    typ := "number",
    onInput.mapToValue --> { v =>
      if v.nonEmpty then
  ...
  private def renderDate(survey: NextIdentificationSurvey): HtmlElement = input(
    cls := "date-input",
    typ := "date",
    onInput.mapToValue --> { v =>
      if v.nonEmpty then
  ...
  private def submitAnswer(survey: NextIdentificationSurvey): Unit = answerValue
    .now()
    .foreach: answer =>
      ...
      val apiCall =
        survey.surveyType match
          case IdentificationSurveyType.Email =>
            ApiClient.answerEmail(answer)
          case IdentificationSurveyType.Profiling =>
            ApiClient.answerProfiling(answer)
