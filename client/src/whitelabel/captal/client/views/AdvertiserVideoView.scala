...
object AdvertiserVideoView:
    ...
    div(
      ...
      child <--
        videoData
          ...
          .map:
            case (Some(video), true) =>
              renderIntroCard(video, showIntro)
            case _ =>
              emptyNode
      ,
  ...
  private def renderIntroCard(video: NextVideo, showIntro: Var[Boolean]): HtmlElement = div(
    cls := "video-intro",
    div(
      ...
      // Warning message
      p(cls := "video-intro-warning", child.text <-- I18nClient.i18n.map(_.video.payAttention)),
        ...
        onMountCallback { ctx =>
          ...
          el.addEventListener(
            ...
            (e: dom.AnimationEvent) =>
              if e.animationName == "introProgress" then
                showIntro.set(false))
        }
      )
    )
  ...
  private def renderVideoContent(
      ...
      isFullscreen: Var[Boolean]): HtmlElement = div(
  ...
  private def renderControls(
      ...
      videoCompleted: Var[Boolean]): HtmlElement = div(
    cls := "video-controls",
    // Play/Pause button
    button(
  ...
  private def playIcon: HtmlElement = span(
    svg.svg(
  ...
  private def pauseIcon: HtmlElement = span(
    svg.svg(
      svg.cls     := "control-icon",
      svg.viewBox := "0 0 24 24",
      svg.fill    := "currentColor",
  ...
  private def fullscreenIcon: HtmlElement = span(
    svg.svg(
      svg.cls     := "control-icon",
      svg.viewBox := "0 0 24 24",
      svg.fill    := "currentColor",
  ...
  private def exitFullscreenIcon: HtmlElement = span(
    svg.svg(
      svg.cls     := "control-icon",
      svg.viewBox := "0 0 24 24",
      svg.fill    := "currentColor",
  ...
  private def renderNoVideo(): HtmlElement = div(
    ...
    button(
      ...
      onClick --> { _ =>
        videoRef
          .now()
          .foreach { v =>
            if videoCompleted.now() then
              markAsWatched(v.currentTime.toInt, completed = true)
          }
      },
      ...
      svg.svg(
        ...
        // Background ring
        svg.circle(svg.cls := "progress-ring-bg", svg.cx := "18", svg.cy := "18", svg.r := "14"),
                ...
                .foreach { v =>
                  val progress =
                    if v.duration > 0 then
                      (v.currentTime / v.duration).min(1.0)
                    else
                      0.0
                  val offset = ((100 * (1 - progress)) * 1000).toInt /
                    1000.0 // Truncate to 3 decimals
      ...
      // Checkmark icon
      span(cls := "progress-checkmark", checkmarkIcon)
    )

  private def checkmarkIcon: SvgElement = svg.svg(
    svg.viewBox        := "0 0 24 24",
    svg.fill           := "none",
  ...
  private def loadNextVideo(
      ...
      videoRef: Var[Option[Video]]): Unit = Runtime.run:
    ApiClient
      .getNextVideo()
      .map:
