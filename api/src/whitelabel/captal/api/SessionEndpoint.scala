...
object SessionEndpoint:
          ...
          case OnMissing.Create(deviceId, locale) =>
            ZIO
              .serviceWithZIO[SessionService](
                _.create(deviceId, locale, Phase.Welcome))
  ...
  val secured: ZPartialServerEndpoint[SessionService, Option[String], SessionData, Unit, ApiError, Unit, Any] =
    endpoint
  ...
  def withPhase(
      phases: Phase*
  ): ZPartialServerEndpoint[SessionService, Option[String], SessionData, Unit, ApiError, Unit, Any] =
    endpoint
      .securityIn(SurveyEndpoints.sessionCookie)
      .errorOut(jsonBody[ApiError])
      .zServerSecurityLogic: cookie =>
        for
          ...
          _ <-
            if phases.contains(session.phase) then ZIO.unit
            else ZIO.fail(ApiError.WrongPhase(session.phase.toString, phases.map(_.toString).toList))
        yield session
