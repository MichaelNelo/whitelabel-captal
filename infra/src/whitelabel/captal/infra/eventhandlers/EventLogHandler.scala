...
object EventLogHandler:
  ...
  private def toEventLogRow(
      ...
      now: String): Option[EventLogRow] =
    extractEventInfo(event).map: (eventType, eventData, occurredAt) =>
      EventLogRow(
  ...
  private def extractEventInfo(event: Event): Option[(String, Json, Instant)] =
    event match
      case Event.User(e)   => extractUserEvent(e)
      case Event.Survey(e) => extractSurveyEvent(e)
      case Event.Video(e)  => extractVideoEvent(e)
        ...
        questionEvent match
          ...
          case QuestionEvent.ProfilingQuestionAnswered(userId, _, questionId, answer, occurredAt) =>
            Some((
              ...
              occurredAt))
          case QuestionEvent
                .LocationQuestionAnswered(userId, _, questionId, hierarchyLevel, answer, occurredAt) =>
            Some((
              ...
              occurredAt))
          case QuestionEvent
                .AdvertiserQuestionAnswered(userId, _, advertiserId, questionId, answer, occurredAt) =>
            Some((
