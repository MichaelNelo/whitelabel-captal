...
/** Standalone entry point for shared provisioning (surveys + advertisers).
  * Used by `captal shared push` via an ephemeral ECS task.
  *
      ...
      .serviceWithZIO[QuillSqlite]: quill =>
        for
          sharedDir <- ZIO.attempt:
            ...
            if dir.isEmpty then throw new RuntimeException("SHARED_DIR is required")
            dir
