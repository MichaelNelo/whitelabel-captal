...
object Templates:
  ...
  def loadAs(resourcePath: String, outputPath: String, vars: Map[String, String] = Map.empty): Template =
    load(resourcePath, vars).copy(path = outputPath)

  /** Load a list of known template files with placeholder substitution. */
  def loadAll(files: List[String], vars: Map[String, String] = Map.empty): List[Template] =
    files.map(load(_, vars))
