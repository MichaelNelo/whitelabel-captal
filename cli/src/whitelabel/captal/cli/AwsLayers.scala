...
object AwsLayers:
  ...
  private def credentialsProvider(aws: CaptalConfig.Aws): Option[StaticCredentialsProvider] = aws
    .accessKeyId
    .zip(aws.secretAccessKey)
    .headOption
    .map: (keyId, secret) =>
      aws.sessionToken match
        case Some(token) =>
          StaticCredentialsProvider.create(AwsSessionCredentials.create(keyId, secret, token))
        case None =>
  ...
  val ecr: ZLayer[CaptalConfig, Throwable, EcrClient] = ZLayer.scoped:
    ZIO.serviceWithZIO[CaptalConfig]: config =>
      val region = Region.of(config.aws.region)
      ZIO.acquireRelease(
        ZIO.attempt {
          val builder = EcrClient.builder().region(region)
          credentialsProvider(config.aws).foreach(builder.credentialsProvider)
          builder.build()
        })(c => ZIO.succeed(c.close()))

  val cloudfront: ZLayer[CaptalConfig, Throwable, CloudFrontClient] = ZLayer.scoped:
    ZIO.serviceWithZIO[CaptalConfig]: config =>
      val region = Region.of(config.aws.region)
      ZIO.acquireRelease(
        ZIO.attempt {
